  ...
  Future<void> _pickFile() async {
    try {
      final result = await FilePicker.platform.pickFiles(
        ...
        allowedExtensions: ['txt', 'md', 'csv', 'json', 'log', 'py', 'dart', 'js', 'html', 'cpp'],
      );
      if (result != null && result.files.single.path != null) {
        final file = File(result.files.single.path!);
        if (await file.length() > 500 * 1024) {
          Get.snackbar('File Too Large', 'Please select a file under 500KB.', 
              backgroundColor: AppColors.orange, colorText: Colors.white);
          return;
        }
        final text = await file.readAsString();
        setState(() {
          _attachments.add({'name': result.files.single.name, 'content': text});
        });
      }
    } catch (e) {
            ...
            Padding(
              ...
              child: Wrap(
                ...
                children: _attachments.map((doc) {
                  return Chip(
                    label: Text(doc['name']!, style: TextStyle(fontSize: 12, color: context.text)),
                    backgroundColor: context.bgHover,
                    deleteIconColor: context.textM,
                    onDeleted: () {
                      setState(() {
            ...
            child: Row(
              ...
              children: [
                ...
                Padding(
                  ...
                  child: IconButton(
                    icon: Icon(Icons.attach_file_rounded, color: context.textM),
                    onPressed: _pickFile,
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      contentPadding: const EdgeInsets.fromLTRB(10, 14, 8, 14),
                    ),
                    onSubmitted: (_) => _send(),
                  ),
                ),
